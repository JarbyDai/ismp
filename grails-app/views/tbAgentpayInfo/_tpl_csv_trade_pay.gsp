...
<g:each in="${tradeList}" status="i" var="trade">${TbAgentpayInfo.batchTypeMap[trade.BATCH_TYPE]}	,<g:formatDate date="${trade.BATCH_SYSDATE}" format="yyyy-MM-dd HH:mm:ss"/>	,${trade.BATCH_ID}	,${trade.DETAIL_ID}	,${trade.TRADE_CUSTORDER},${trade.TRADE_CARDNAME}
    ,${TbAgentpayDetailsInfo.tradeStatusMap[trade.TRADE_STATUS]},${TbAgentpayDetailsInfo.tradeFeedbackcodeMap[trade.TRADE_FEEDBACKCODE]},<g:formatNumber currencyCode="CNY" number="${trade.TRADE_FEE}" type="currency"/>		,${TbAgentpayDetailsInfo.feeTypeMap[trade.BATCH_FEETYPE]},<g:formatNumber currencyCode="CNY" number="${trade.TRADE_AMOUNT}" type="currency"/>
    ,${trade.BATCH_REMARK1.toString()},${trade.TRADE_REASON},${TbAgentpayDetailsInfo.tradeRefuedMap[trade.TRADE_REFUED]}
</g:each>
#-------------------------------------------------------------------------------------------
