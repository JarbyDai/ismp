...
//        cmCustomerOperator.save(flush: true)
        ...
        def content='重置登录密码确认：您本次重置登录密码操作的验证码是'+mobile_captcha+'。【合利宝】'
        sendMobileCaptcha(cmCustomer,cmCustomerOperator,sendTo,content,mobile_captcha,'reset_login_pass')
    }

    def LoginByMobileCaptcha(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo) throws Exception {
        //1随机产生6位数字手机验证码
        def mobile_captcha=KeyUtils.getRandNumberKey(6)
        def content=new Date().format("MM月dd日")+',您本次登录商户管理平台的短信验证码是'+mobile_captcha+'。【合利宝】'
        try{
            sendMobileCaptcha(cmCustomer,cmCustomerOperator,sendTo,content,mobile_captcha,'ismp_login')
        }
        catch (Exception e){
            throw new RuntimeException("获取短信验证码通信暂时中断，稍后再试");
        }
    }
    ...
    def sendMobileCaptcha(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo,String content,String mobile_captcha,String useType)  throws Exception
    {
        //1.1写动态口令表
        def cmDynamicKey=new CmDynamicKey()
        cmDynamicKey.customer=cmCustomer
        ...
        cmDynamicKey.timeExpired=new Date(System.currentTimeMillis()+5*60*1000)                                       //过期时间,设为3分钟后
        //cmDynamicKey.timeUsed=''                                                                                   //使用时间
        cmDynamicKey.isUsed=false                                                                                  //是否使用过
        cmDynamicKey.useType=useType                                                                                //使用类型
        cmDynamicKey.save(flush: true,failOnError:true)
        log.info "短信验证码生成成功！！！"
        log.debug "验证码为："+cmDynamicKey.verification
...
//                 writeInfoPage "发送成功"
                 ...
                 throw new RuntimeException("用户未授权，获取短信验证码失败")
          } else {
...
//        }
    ...
    def sendBindMobileSMS(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo){
        //1随机产生6位数字手机验证码
        def mobile_captcha=KeyUtils.getRandNumberKey(6)
        def content=new Date().format("MM月dd日")+',您申请绑定手机号。手机校验码：'+mobile_captcha+'。【合利宝】'
    ...
    def resetPayPassByMobile(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo){
        //1随机产生6位数字手机验证码
        def mobile_captcha=KeyUtils.getRandNumberKey(6)
        def content='修改支付密码确认：您本次修改支付密码操作的验证码是'+mobile_captcha+'。【合利宝】'
   ...
   def updateMobile(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo){
        //1随机产生6位数字手机验证码
        def mobile_captcha=KeyUtils.getRandNumberKey(6)
        def content=new Date().format("MM月dd日")+',您申请修改绑定手机号。手机校验码：'+mobile_captcha+'。【合利宝】'
   ...
   def newMobile(CmCustomer cmCustomer,CmCustomerOperator cmCustomerOperator,String sendTo){
        //1随机产生6位数字手机验证码
        def mobile_captcha=KeyUtils.getRandNumberKey(6)
        def content=new Date().format("MM月dd日")+',您绑定新的手机号。手机校验码：'+mobile_captcha+'。【合利宝】'
   ...
   def saveUpdateMobile(CmDynamicKey cmDynamicKey,CmCustomerOperator cmCustomerOperator){
    ...
    //查询短信验证码是否存在
    def queryCaptcha(CmLoginCertificate loginCertificate, String captcha, String useType) {
        //验证提交的信息是否为空
        if (!loginCertificate.customerOperator.defaultMobile || !loginCertificate.customerOperator.defaultMobile .matches('^[1][3,4,5,7,8][0-9]{9}$') || !captcha || loginCertificate.customerOperator.defaultMobile .length() != 11) {
            log.info("用户: ${loginCertificate.customerOperator.defaultEmail }的手机号不正确")
            return false
        }
        //检查手机校验码
        def query = {
            eq('useType', useType)
            eq('customer',loginCertificate.customerOperator.customer)
            eq('parameter',loginCertificate.customerOperator.id.toString())
            eq('sendType', 'mobile')
            eq('sendTo', loginCertificate.customerOperator.defaultMobile)
            eq('isUsed', false)
            ge("timeExpired", new Date())
        }
        def cmDynamicKeyList = CmDynamicKey.createCriteria().list([sort: "id", order: 'desc'], query)
        CmDynamicKey cmDynamicKey = (cmDynamicKeyList && cmDynamicKeyList.size() > 0) ? cmDynamicKeyList.first() : null
        if (ConfigurationHolder.config.verifyCaptcha != 'false' && ((!cmDynamicKey) || (captcha != cmDynamicKey.verification))) {
