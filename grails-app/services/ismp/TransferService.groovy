...
class TransferService {
    ...
    def transfer(CmDynamicKey cmDynamicKey, CmLoginCertificate payer, CmCustomerOperator cmCustomerOperator_payer, CmLoginCertificate payee, Long amount, String transferType, String subject, String note, String IP, String delayTime) {
        ...
        //判断是否在定义的规定时间
        if (!(delayTime in TradeTransfer.delayTimeMap.keySet().toList())) {
            return
        }
        ...
        tradeTransfer.delayTime = delayTime
        tradeTransfer.startTransferTime = DateUtils.addTime(today, 1000 * 3600 * Integer.parseInt(delayTime))
        ...
        if (tradeTransfer.transferModel == 'close') {
            if("0".equals(delayTime)){
                ...
                if (res)//远程调用成功
                {
                    ...
                    cmTransactionQuotaService.quotaSuccessHandle(cmCustomer_payer.customerNo, 'TRANSFER', amount/100)
