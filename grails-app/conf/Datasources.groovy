...
datasources = {
    datasource(name: 'ismp') {
        domainClasses([
                ...
                ismp.Merchant,
                ismp.CmCustomerCertificateRecord,
                ismp.UserWalletTrade,
                ismp.UserOrder,
                ismp.TradeAttached,
                ismp.CmTransactionQuota
        ])
    ...
    datasource(name: 'risk') {
        domainClasses([
                risk.RiskBlackParams,
                risk.RiskBlackLog
        ])
        driverClassName('oracle.jdbc.OracleDriver')
        url(ConfigurationHolder.config.dataSource.risk.url)
        username(ConfigurationHolder.config.dataSource.risk.username)
        password(DESCodec.decode(ConfigurationHolder.config.dataSource.risk.password))
        println "risk:"+DESCodec.decode(ConfigurationHolder.config.dataSource.risk.password)
        dbCreate(ConfigurationHolder.config.dataSource.risk.dbCreate)
        pooled(true)
        logSql(false)
        dialect(org.hibernate.dialect.Oracle10gDialect)
        hibernate {
