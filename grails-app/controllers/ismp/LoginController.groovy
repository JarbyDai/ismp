...
import risk.RiskBlackLog
import risk.RiskBlackParams
import role.Role
import model.Model
...
class LoginController {
    ...
    def login = {
        ...
        render(view: "login", model: [boNews: list, newsCount: newsCount, boNews1: list1, boNews2: list2, servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
...
//        if(cookie)cookie[0].setMaxAge(0);//让cookie过期
    ...
    def authenticate = {
    //    println params
    //    params.loginname = new String(Base64.DecodeBase64(params.loginname))
        if (request.method.toLowerCase() == 'get') { redirect(action: "login"); return}
        def errmsg = ''

     //   log.info "loginpwd:${params.loginpwd} params.captcha:${params.captcha}"
        ...
        if (grailsApplication.config.verifyCaptcha != 'false') {
            ...
            if (!params.captcha || !session.captcha?.isCorrect(params.captcha.toUpperCase())) {
                ...
                errmsg = '验证码不正确，请重新输入'
                log.info errmsg
                render(view: "login", model: [errmsg: errmsg, boNews: list, boNews1: list1, boNews2: list2, servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
                return
            }
        }
        session.captcha = null
        ...
        if (!loginCertificate) {
         ...
         if(!params.mobileCaptcha || !StringUtils.isNumeric(params.mobileCaptcha) || params.mobileCaptcha.length()!=6){
               errmsg = "短信验证码为空或不是数字或长度不正确！！！！！"
               render(view: "login", model: [errmsg: errmsg, boNews: list, boNews1: list1, boNews2: list2,servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
               return
         }
        ...
        RiskBlackParams riskBlackParams = RiskBlackParams.findWhere(['paramsType' : '4', 'paramsValue' : ip])
        if(riskBlackParams){
            RiskBlackLog riskBlackLog = new RiskBlackLog()
            riskBlackLog.customerNo = cmCustomer.customerNo
            riskBlackLog.customerType = cmCustomer.type
            riskBlackLog.paramsType = '4'
            riskBlackLog.paramsValue = ip
            riskBlackLog.message = "ip：${ip}存在IP黑名单中"
            riskBlackLog.ip = localIp
            riskBlackLog.createTime = new Date()
            riskBlackLog.save(failOnError: true,flush: true)
            errmsg = "无法登陆"
            render(view: "login", model: [errmsg: errmsg, boNews: list, boNews1: list1, boNews2: list2,servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
            return
        }
        ...
        String RSAEncryptedDataBase64 = params.clientrandom;
        String RSBase64 = "MDEyMzQ1Njc4OWFiY2RlZg==";
        String tripleDESEncryptedDataBase64 = params.loginpwd;
//        log.info "loginpwd:${tripleDESEncryptedDataBase64}"
...
//            log.info "Base64 Password:"+plainPWDBase64

            str_pwd = cmCustomerOperator.id + plainPWDBase64
        }catch (Exception e){
            e.printStackTrace();
        }
        ...
        if (str_pwd.encodeAsSHA1() == cmCustomerOperator.loginPassword) {
            ...
            Boolean b =operatorService.queryCaptcha(loginCertificate,params.mobileCaptcha,"ismp_login")
            if(b==false) {
                errmsg = "短信验证码不正确,请重新输入！！！！！"
                render(view: "login", model: [errmsg: errmsg, boNews: list, boNews1: list1, boNews2: list2,servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
                return
            }
...
//                def modelsList = Model.createCriteria().list{order('modelindex','asc')}
            ...
            if (cmCustomerOperator.loginErrorTime >= 5) {
                ...
                cmCustomerOperator.save(flush: true, failOnError: true)
                if(cmCustomerOperator.loginErrorTime == 1){
                    errmsg = "账户名或登录密码不正确，请重新输入。"
                }else{
                    errmsg = "账户名或登录密码不正确，还有"+(5-cmCustomerOperator.loginErrorTime)+"次机会。"
                }
                log.info errmsg
                render(view: "login", model: [errmsg: errmsg, boNews: list, boNews1: list1, boNews2: list2,servernum:"MDEyMzQ1Njc4OWFiY2RlZg=="])
            }
    ...
    def saveresetloginpass = {
        ...
        if (method == 'email') {
            ...
            if (grailsApplication.config.verifyCaptcha != 'false' && (!mobile_captcha || mobile_captcha != cmDynamicKey.verification)) {
                ...
                //验证码验证通过
                operatorService.updateLoginPass(cmDynamicKey, params.password)
                writeInfoPage "设置登录密码成功，请登录系统，在我的账户—>账户信息->安全设置->支付密码，设置支付密码。", 'ok'
            }
        }
    }
    ...
    def resetlogin = {
        def loginCertificate = CmLoginCertificate.findByLoginCertificate(params.loginname as String)
        if (!loginCertificate) {
    ...
    def sendCaptcha = {
        ...
        def content = new Date().format("MM月dd日") + ',您设置了登录密码。手机校验码：' + mobile_captcha + '。【合利宝】'
    ...
    def getMobileCaptcha={
        //验证用户名不能为空
        if(!params.loginname){
            render([status: false, reason: "用户名不能为空！！！"] as JSON)
            return
        }
        else{
           // params.loginname = new String(Base64.DecodeBase64(params.loginname))
            ...
            def loginCertificate = CmLoginCertificate.findByLoginCertificate(params.loginname.toLowerCase())
            if (!loginCertificate) {
                render([status: false, reason: "用户名不正确或不存在，请您重新输入！！！"] as JSON)
                return
            }
            else{
                def cmCustomerOperator = loginCertificate.customerOperator
                ...
                String errmsg=""
                if (!cmCustomer) {
                    errmsg= "您没有分配角色，无法获取短信验证码！！！"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                else if (!cmCustomerOperator.status.equals("normal")) {
                    errmsg = "对不起，您的账户已被停用。不能获取短信验证码！！！"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                else if (cmCustomerOperator.status.equals('locked')) {
                    errmsg = "对不起，您的账号已被锁定。不能获取短信验证码"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                else if (cmCustomerOperator.status.equals('disabled')) {
                    errmsg = "对不起，您的账号已被停用。不能获取短信验证码"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                else if (cmCustomerOperator.status.equals('deleted')) {
                    errmsg = "对不起，您输入帐户名不存在。不能获取短信验证码"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                ...
                if(!cmCustomerOperator.defaultMobile){
                    errmsg = "对不起，您还没有绑定手机号码。请您先点击“忘记密码”功能，在绑定的邮箱中去重置手机号。后再点击获取短信验证码！！！。"
                    log.info errmsg
                    render([status: false, reason: errmsg ] as JSON)
                    return
                }
                    ...
                    //获取操作员绑定的手机号码
                     def mobile=cmCustomerOperator.defaultMobile.trim().toString()
                    try{
                        operatorService.LoginByMobileCaptcha(cmCustomerOperator.customer, cmCustomerOperator, mobile)
                        errmsg = "短信验证码已经发送，请您留意查收。"
                        log.info errmsg
                        render([status: "0000000000", reason: errmsg ] as JSON)
                        return
                    }
                    catch (Exception e)
                    {
                        errmsg = "对不起，获取短信验证码通信暂时中断，稍后再试！！！。"
                        log.info errmsg
                        render([status: false, reason: errmsg ] as JSON)
                        return
                    }
