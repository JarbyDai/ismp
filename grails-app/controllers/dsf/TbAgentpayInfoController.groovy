...
class TbAgentpayInfoController {
    ...
    def agentpay = {
        ...
        if (cmCustomer.status == 'init') {
            writeInfoPage "用户状态错误，不能做代付申请操作，请联系合利宝客服。"
            return
        }
        if (cmCustomer.status in ['disabled', 'deleted']) {
            writeInfoPage "对不起，您不能做代付申请操作，请联系合利宝客服。", 'warn'
    ...
    def agentpaylist = {
        ...
        if (!params.endDate) {
            params.endDate = DateUtils.getDateStr(new Date())
        }
        if (!params.startDate) {
        ...
        if (DateUtils.equalsDate(params.startDate, params.endDate, pattern) == 1) {
            errmsg = "开始时间不能大于结束时间！！！！！"
            render(view: "agentpaylist", model: [errmsg: errmsg, tradeList: null, tradeListTotal: 0])
            return
        }
        ...
        def query ={
            ...
            eq('batchStatus', '1')
            eq('batchUsrChk', '1')
            ge('batchDate', params.startDate)
            le('batchDate', params.endDate)
            if (params.tradeNo)
        ...
        if (isDownload) {
            ...
            response.setCharacterEncoding("GBK")
            response.setHeader("Content-disposition", "attachment; filename=${filename}.${params.format}")
            render(template: "tpl_${params.format}_trade_pay", model: [tradeList: list, tradeListTotal: list.size(), totalamount: totalAmount])
        } else {
            def total = TbAgentpayInfo.createCriteria().get(){
                and query
                projections{
                    rowCount()
                    sum('batchAmount')
    ...
    def agentpayhis = {
        ...
        params.offset = params.offset ? params.int('offset') : 0
        boolean isDownload = false;
        if (params?.format && params.format == 'xls') {
            params.offset = 0
            params.max = 50000
            isDownload = true
        }
        ...
        if (DateUtils.equalsDate(params.startDate, params.endDate, pattern) == 1) {
            errmsg = "开始时间不能大于结束时间！！！！！"
            render(view: "agentpayhis", model: [errmsg: errmsg, tradeList: null, tradeListTotal: 0])
            return
        }
        if (!DateUtils.isCorrectGapMonth(params.startDate, params.endDate, 3, pattern)) {
            errmsg = "您查询的时间范围不能大于三个月时间！！！！！"
        ...
        String startTime = params.startDate
        String endTime = params.endDate

        def sql = new Sql(dataSource_dsf)
        def query = """
                       SELECT t3.trade_feedbackcode,t3.trade_accountname,t3.trade_cardname,t3.trade_custorder,t3.DETAIL_ID,t3.trade_status,t3.trade_amount,t3.trade_fee,t3.trade_Reason,t3.trade_Refued,t1.batch_id,t1.batch_type,t1.batch_date,t1.batch_sysdate,
                        t1.batch_count,t1.batch_amount,t1.batch_status,t1.batch_fee,t1.batch_feetype,t1.batch_accamount,t1.batch_remark1,t1.batch_filename
                        FROM  Tb_agentpay_details_Info t3  left join tb_agentpay_info t1 on  t3.batch_id=t1.batch_id
                        WHERE t1.batch_bizid = ${session.cmCustomer.customerNo}
                         AND t1.batch_type= 'F'
                        ${(params.batchStatus && params.batchStatus != null) ? " AND t3.trade_status = '" + params.batchStatus.trim() + "'" : ""}
                        ${(params.tradeCardname && params.tradeCardname != null) ? " AND t3.trade_cardname like '%" + params.tradeCardname.trim() + "%'" : ""}
                        ${(params.tradeRefued && params.tradeRefued != null) ? " AND t3.trade_Refued = '" + params.tradeRefued.trim() + "'" : ""}
                        ${(params.tradeReason && params.tradeReason != null) ? " AND t3.trade_Reason like '%" + params.tradeReason.trim() + "%'" : ""}
                        ${(params.custorder && params.custorder != null) ? " AND t3.trade_custorder = '" + params.custorder.trim() + "'" : ""}
                        ${(params.detatlNo && params.detatlNo != null) ? " AND t3.DETAIL_ID = '" + params.detatlNo.trim() + "'" : ""}
                        ${(params.tradeNo && params.tradeNo != null) ? " AND t3.batch_id = '" + params.tradeNo.trim() + "'" : ""}
                        AND t1.batch_date>='${startTime}' AND t1.batch_date<='${endTime}'
                        UNION All
                        SELECT t4.trade_feedbackcode,t4.trade_accountname,t4.trade_cardname,t4.trade_custorder,t4.DETAIL_ID,t4.trade_status,t4.trade_amount,t4.trade_fee,t4.trade_Reason,t4.trade_Refued,t2.batch_id,t2.batch_type,t2.batch_date,t2.batch_sysdate,t2.batch_count,
                        t2.batch_amount,t2.batch_status,t2.batch_fee,t2.batch_feetype,t2.batch_accamount,t2.batch_remark1,t2.batch_filename
                        FROM Tb_Autoagent_Details_Info t4 left join tb_autoagent_info t2 on  t4.batch_id=t2.batch_id
                        WHERE t2.batch_bizid = ${session.cmCustomer.customerNo}
                          AND t2.batch_type= 'F'
                        ${(params.batchStatus && params.batchStatus != null) ? " AND t4.trade_status = '" + params.batchStatus.trim() + "'" : ""}
                        ${(params.tradeCardname && params.tradeCardname != null) ? " AND t4.trade_cardname like '%" + params.tradeCardname.trim() + "%'" : ""}
                        ${(params.tradeRefued && params.tradeRefued != null) ? " AND t4.trade_Refued = '" + params.tradeRefued.trim() + "'" : ""}
                        ${(params.tradeReason && params.tradeReason != null) ? " AND t4.trade_Reason like '%" + params.tradeReason.trim() + "%'" : ""}
                        ${(params.tradeNo && params.tradeNo != null) ? " AND t4.batch_id = '" + params.tradeNo.trim() + "'" : ""}
                        ${(params.custorder && params.custorder != null) ? " AND t4.trade_custorder = '" + params.custorder.trim() + "'" : ""}
                        ${(params.detatlNo && params.detatlNo != null) ? " AND t4.DETAIL_ID = '" + params.detatlNo.trim() + "'" : ""}
                        AND t2.batch_date>='${startTime}' AND t2.batch_date<='${endTime}'
                        ORDER BY batch_sysdate DESC
                    """
        def queryList = """
                      SELECT * FROM (${query}) t
                  """
        def queryTotal = """
                      SELECT COUNT(1) total_count, SUM(t.TRADE_AMOUNT) total_sum FROM (${query}) t ${isDownload ? "WHERE ROWNUM<=" + params.max : ""}
                   """
        def count = 0
        def totalamount = 0
        HibernateTemplate ht = DatasourcesUtils.newHibernateTemplate('dsf')
        def result = ht.executeFind({ Session session ->
            def sqlQuery = session.createSQLQuery(queryList.toString())
            def countSession = session.createSQLQuery(queryTotal.toString())
        ...
        if (isDownload) {
            ...
            render(template: "tpl_${params.format}_trade_pay", model: [tradeList: result, tradeListTotal: count, totalamount: totalamount])
        } else {
...
//                    }
                    ...
                    // }
                    if (params.id) {
                        TbAgentpayDetailsInfo agentpayDetails = TbAgentpayDetailsInfo.get(params.long("id"))
                        TbAutoagentDetailsInfo autoagentDetails = TbAutoagentDetailsInfo.get(params.long("id"))
                        if (agentpayDetails) {
                            eq('batch', TbAgentpayInfo.load(TbAgentpayDetailsInfo.get(params.long("id")).batch.id))
                        }
                        if (autoagentDetails) {
                            eq('batch', TbAutoagentInfo.load(TbAutoagentDetailsInfo.get(params.long("id")).batch.id))
                        }
...
//                    }
    ...
    def detaillist = {
        ...
        TbAgentpayDetailsInfo agentpayDetails = TbAgentpayDetailsInfo.get(params.long("id"))
        TbAutoagentDetailsInfo autoagentDetails = TbAutoagentDetailsInfo.get(params.long("id"))
        def tbAgentpayInfo = null
        def tbAutoagentInfo = null
        if (agentpayDetails) {
            tbAgentpayInfo = agentpayDetails.batch
        }
        if (autoagentDetails) {
            tbAutoagentInfo = autoagentDetails.batch
        }

        def type = ''
        if (tbAgentpayInfo)
            type = tbAgentpayInfo.batchType
        if (tbAutoagentInfo)
            type = tbAutoagentInfo.batchType
        ...
        if (params?.format && params.format in ["txt", "csv"]) {
            ...
            if (!list)
                list = TbAutoagentDetailsInfo.createCriteria().list(params, query)
            def count = TbAgentpayDetailsInfo.createCriteria().count(query)
            if (count < 1)
                count = TbAutoagentDetailsInfo.createCriteria().count(query)
            def totalamount = list.sum { it.tradeAmount }
            response.contentType = ConfigurationHolder.config.grails.mime.types[params.format]
            def batchFilename = ""
            if (tbAgentpayInfo) {
                batchFilename = tbAgentpayInfo.batchFilename//.fin.findAll(list.get(0)).batchFilename
            }
            if (tbAutoagentInfo) {
                batchFilename = tbAutoagentInfo.batchFilename//.fin.findAll(list.get(0)).batchFilename
            }
            def filename
            ...
            try {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    sheet.addCell(label)
                    label = new jxl.write.Label(excelCol++, row, rr.tradeCustorder)
                    sheet.addCell(label)
                    ...
                    sheet.addCell(label);
                    label = new jxl.write.Label(excelCol++, row, rr.tradeBranchbank != 'null' ? rr.tradeBranchbank : '');
                    sheet.addCell(label);
                    label = new jxl.write.Label(excelCol++, row, rr.tradeSubbranchbank != 'null' ? rr.tradeSubbranchbank : '');
                    sheet.addCell(label);
    ...
    def simplepay = {
        def cmCustomer = CmCustomer.get(session.cmCustomer.id)
        if (cmCustomer.status == 'init') {
            writeInfoPage "用户状态错误，不能做单笔代付操作，请联系合利宝客服。"
            return
        }
        if (cmCustomer.status in ['disabled', 'deleted']) {
            writeInfoPage "对不起，您不能做单笔代付操作，请联系合利宝客服。", 'warn'
            session.cmCustomer = null
            return
        }
    }
