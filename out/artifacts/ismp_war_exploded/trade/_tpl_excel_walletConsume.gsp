...
<div id="template_26326" align=center x:publishsource="Excel">
    ...
    <table border=0 cellpadding=0 cellspacing=0 width=1524 class=xl637821
        ...
        <g:each in="${tradeList}" status="i" var="trade">
            <tr height=18 style='height:13.5pt'>
                <td class=xl677821 style='border-left:none'>${UserWalletTrade.tradeMap[trade?.tradeType]}</td>
                <td class=xl677821 style='border-left:none'>${trade?.reqDate.format("yyyy-MM-dd HH:mm:ss")}</td>
                <td class=xl677821 style='border-left:none'>${trade?.orderId}</td>
                <td class=xl677821 style='border-left:none'>${trade?.transNo}</td>

                <td class=xl657821 align=right style='border-left:none'>${trade.targetName}</td>
                ...
                <td class=xl657821 align=right style='border-left:none'>${UserWalletTrade.statusMap[trade.status]}</td>
                <td class=xl657821 align=right style='border-left:none'><strong class="hsfong"><g:if test="${UserOrder.findByOrderId(trade.orderId)?.refundedAmount>0}"><g:formatNumber currencyCode="CNY" number="${UserOrder.findByOrderId(trade.orderId)?.refundedAmount/100}" type="currency"/></g:if></strong></td>
