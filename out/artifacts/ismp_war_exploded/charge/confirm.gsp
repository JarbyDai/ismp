    ...
    <form action="create" method="post" name="actionForm" id="actionForm" target="_blank">
        ...
        <div class="txbox">
            ...
            <dl>
              ...
              <dt>充值金额：</dt>
              <dd><input name="amount" id="amount" type="text" maxlength="13"/>&nbsp;元</dd>
            </dl>
            <div class="xybbtn">
       ...
       $(document).ready(function() {
           if (checkIE())  {
               plugins = new CFCA_CONTROL({
                   ...
                   'CryptoKitUrl_X64' : "${resource(dir: 'ocx', file: 'CryptoKit.helipay.x64.cab')}"
               });
           }else {
               alert("请使用IE浏览器");
           }
            ...
            $("#nextStep").click(function(){
              ...
              if(confirm("确定充值吗？")) {
                  ...
                  if(!plugins.checkCryptoKitActiveX() || !plugins.checkCryptoAgentActiveX()){
                      alert("请确保安全控件已正确安装！");
                      return;
                  }
                  ...
                  var signSource = "bankname:"+$("#bankname").val();
                  signSource += "|buyer_name:"+$("#buyer_name").val();
                  signSource += "|buyer_id:"+$("#buyer_id").val();
                  signSource += "|preference:"+$("#preference").val();
                  if(${(params.channel?.encodeAsHTML() == 'SPDB_B2B')||(params.channel?.encodeAsHTML() == 'CMBC_B2B')}){
                      signSource += "|payCusNo:"+$("#payCusNo").val();
                  }
                  if(${params.channel?.encodeAsHTML() == 'BOCM_B2B'}){
                      signSource += "|amount:"+$("#payCusNoid").val();
                  }
                  signSource += "|amount:"+$("#amount").val();
