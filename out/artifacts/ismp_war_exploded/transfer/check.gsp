<%@ page import="ismp.RefundBatch; ismp.RefundAuth; ismp.TradeRefund; ismp.TradeBase; trade.AccountCommandPackage;" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8"/>
    <meta name="layout" content="main"/>
    <title>合利宝-转账审核</title>
...
<div class="trnmenu"> <span class="left trnmenutlt">交易管理：</span>
  <div class="rtnms">
    <ul>
        ...
        <li>
            <a href="${request.contextPath}/transfer/list">转账记录</a>
        </li>
        <li class="rtncnt blue">转账审核</li>
        %{--<li>--}%
...
<form action="check" method="post" name="searchform" id="searchform" style="width:960px; margin:0 auto">
    <div class="serch">
      <p>搜索</p>
    ...
    <div class="serchlitst">
        ...
        <table class="tlb1">
          <tr>
            ...
            <th class="txtCenter" scope="col">到账时间</th>
            ...
            <th class="txtCenter" scope="col">全选<input type="checkbox" id="chbox" name="chbox" onclick="selectAllCheckBox();"></th>
          </tr>
            <g:if test="${params.flag == '1'}">
                <g:each in="${tradeList}" status="i" var="trade">
                    <tr>
                        ...
                        <td scope="col" class="blue">${trade.startTransferTime?.format("yyyy-MM-dd HH:mm:ss")}</td>
                        ...
                        <td class="txtCenter" scope="col">
                            <a href="${request.contextPath}/transfer/checkDetail/${trade.id}" style="color:blue">详细</a>|
                            <a href="${request.contextPath}/transfer/detail/${trade.id}" style="color:blue">通过</a>
                        </td>
                        <td class="txtCenter" scope="col"><input type='checkbox' name='box' id='box'></td>
                        <g:hiddenField name="id" value="${trade.id}"></g:hiddenField>
                    </tr>
                </g:each>
            </g:if>
            <g:else>
                <g:each in="${tradeList}" status="i" var="trade">
                    <tr>
                        ...
                        <td class="txtCenter" scope="col">${trade.payeeCode}</td>
                        <td scope="col" class="blue">${trade.dateCreated.format("yyyy-MM-dd HH:mm:ss")}</td>
                        <td scope="col" class="blue">${trade.startTransferTime ? trade.startTransferTime?.format("yyyy-MM-dd HH:mm:ss") : trade.dateCreated.format("yyyy-MM-dd HH:mm:ss")}</td>
                        <td class="txtCenter" scope="col"><strong style="color:red"><g:formatNumber currencyCode="CNY" number="${trade.amount/100}" type="currency"/></strong></td>
                        <td class="txtCenter" scope="col">${trade.subject}</td>
                        <td class="txtCenter" scope="col">${trade.note}</td>
                        <td class="txtCenter" scope="col">${trade.statusMap[trade.status]}</td>
                        <td class="txtCenter" scope="col">
                            <a href="${request.contextPath}/transfer/checkDetail/${trade.id}" style="color:blue">详细</a>|
                            <a href="#" onclick="singlePass(${trade.id})" style="color:blue">通过</a>
                        </td>
                        <td class="txtCenter" scope="col"><input type='checkbox' name='box1' id='box1'></td>
                        <g:hiddenField name="id1" value="${trade.id}"></g:hiddenField>
                    </tr>
                </g:each>
            </g:else>
        </table>
            ...
            <table align="center">
                <tr>
                    ...
                    <g:if test="${session.level3Map != null && session.level3Map['trade/refundPass'] != null}">
                        <input type="button" onclick="checkPass();" class="serchbtn" value="审核通过">
                    </g:if>
                </tr>
            </table>
        </g:if>
...
//        alert(obj.value);
    ...
    function selectAllCheckBox() {
        ...
        if (document.getElementById("chbox").checked) {
            if (flag == '1') {
                ...
                var len = document.getElementsByName("box1").length;
                for (i = 0; i < len; i++) {
                    document.getElementsByName("box1")[i].checked = true;
                }
            }
        }
        else {
            if (flag == '1') {
                ...
                var len = document.getElementsByName("box1").length;
                for (i = 0; i < len; i++) {
                    document.getElementsByName("box1")[i].checked = false;
                }
            }
        }
    }
    ...
    function checkPass() {
        ...
        var id = "";
        if (flag == '1') {
            len = document.getElementsByName("box").length;
            for (i = 0; i < len; i++) {
            ...
            len = document.getElementsByName("box1").length;
            for (i = 0; i < len; i++) {
                if (document.getElementsByName("box1")[i].checked) {
                    id = id + document.getElementsByName("id1")[i].value + ",";
                    sign = 1
                }
            }
        }
        ...
        if (sign == 1) {
            if (confirm("您确认要审批通过？")) {
                window.location.href = '${createLink(controller:'transfer', action:'checkPass', params:['statusFlag':'1'])}&id=' + id;
            }
        }
    }
    //单笔通过
    function singlePass(str) {
        if (confirm("您确认要审批通过？")) {
            window.location.href = '${createLink(controller:'transfer', action:'checkPass', params:['statusFlag':'1'])}&id=' + str;
        }
    }
</script>
</body>
