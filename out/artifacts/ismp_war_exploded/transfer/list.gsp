        ...
        <table class="tlb1">
          ...
          <g:if test="${params.flag == '1'}">
              ...
              <tr>
                ...
                <td class="txtCenter" scope="col">
                    <g:if test="${trade?.transferModel=='open'}">
                        <g:if test="${trade?.status=='starting'}">待审批</g:if>
                        <g:elseif test="${trade?.status=='closed'}">审批拒绝</g:elseif>
                        <g:else>审批完成</g:else>
                    </g:if>
                </td>
          ...
          <g:else>
              ...
              <tr>
                ...
                <td class="txtCenter" scope="col">
                    <a href="${request.contextPath}/transfer/detail/${trade.id}" style="color:blue">详细</a>
                    <g:if test="${trade.status == 'processing'}">
                         |  <a href="#" onclick="cancelTransfer(${trade.id})" style="color:blue">取消</a>
                    </g:if>
                </td>
              </tr>
              </g:each>
...
//        alert(obj.value);
    ...
    function cancelTransfer(id){
        if (confirm("您确认要取消该笔转账？")) {
            window.location.href = '${createLink(controller:'transfer', action:'cancel')}?id=' + id;
        }
    }
</script>
</body>
