...
<form action="${createLink(controller: 'transfer', action: 'step2')}" method="post" name="actionForm" id="actionForm">
  <div class="txbox">
  	<dl>
        ...
        <g:if test="${acAccount_Main!=null&&acAccount_Main.balance>0}">
            ...
            <dt>付款金额：</dt>
            <dd><input name="amount" id="amount" type="text" maxLength="13" onkeyup="if(isNaN(value))execCommand('undo')" onafterpaste="if(isNaN(value))execCommand('undo')" value="${params.amount}" data-explain="对个人限2000.00元，对企业限1000000.00元。"/>元</dd>
    ...
    <div class="xybbtn">
        <g:if test="${acAccount_Main!=null&&acAccount_Main.balance>0}">
    	    <input type="submit" class="btn mglf10" onclick="return checkSubmit();" value="下一步" />
        </g:if>
    </div>
  </div>
 </form>
...
<script>
    ...
    //判断转账金额是否超越限制金额
    function checkmoney(){
        var transfAmount = $("#amount").val();
        if(transfAmount==''){
            alert("转账金额不能为空");
            return false;
        }
        var exp = /^([1-9][\d]{0,7}|0)(\.[\d]{1,2})?$/;
        if(!exp.test(transfAmount)){
            alert("输入金额格式不正确");
            return false;
        }
        if(B2BSign==1){
            if(transfAmount>1000000){
                alert("付款金额不能大于1000000元");
                return false;
            }
        }else{
             if(transfAmount>2000){
                 alert("付款金额不能大于2000元");
                 return false;
            }
        }
    ...
    function checkAccount(){
        ...
        if(!reg.test(toAccount)){
            alert("格式不正确，账户是email地址");
            return false;
        }
        return true;
    }
